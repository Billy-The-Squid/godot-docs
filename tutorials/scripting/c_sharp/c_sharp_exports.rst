...
Exported properties can specify a default value; otherwise, the `default value of the type <https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/default-values>`_ is used instead.
...
Properties whose type is an enum with the ``[Flags]`` attribute can be exported and
...
A flags enum uses powers of 2 for the values of the enum members. Properties that
...
Properties whose type is an enum can be exported and their values are limited to the members
...
Integer and string properties can also be limited to a specific list of values using the
